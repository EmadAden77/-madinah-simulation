...
function noise2(lon,lat,scale=1){const a=Math.sin((lon*511.73+lat*829.11)*scale),b=Math.sin((lon*1731.17-lat*1291.37)*scale*.63),c=Math.sin((lon*6321.9+lat*2711.4)*scale*.21),d=Math.cos((lon*11129.2-lat*7439.4)*scale*.11);return(a+b*.55+c*.28+d*.16)/1.99}
...
for(const[cx,cy,count,sx,sy]of clusters){for(let i=0;i<count;i++,ci++){
  let lon=cx+(seed(ci*23+1)-.5)*sx,lat=cy+(seed(ci*23+2)-.5)*sy;
  if(distM(lon,lat,center.lon,center.lat)<72){i--;continue}
  compounds.push({lon,lat,w:13+seed(ci*23+3)*22,h:11+seed(ci*23+4)*19,ang:(seed(ci*23+5)-.5)*.36,parts:2+Math.floor(seed(ci*23+6)*4),tone:seed(ci*23+7),yard:seed(ci*23+8)>.22,wall:seed(ci*23+9)>.38,id:ci})
}}
...
function drawLine(buf,x0,y0,x1,y1,width,color,a=255){const dx=x1-x0,dy=y1-y0,steps=Math.max(1,Math.ceil(Math.hypot(dx,dy)));for(let i=0;i<=steps;i++){const t=i/steps;drawDisc(buf,x0+dx*t,y0+dy*t,width/2,color,a)}}
...
function drawOutlinedRect(buf,cx,cy,w,h,ang,outer,inner,wallPx){drawRotRect(buf,cx,cy,w,h,ang,outer,255);drawRotRect(buf,cx,cy,Math.max(1,w-wallPx*2),Math.max(1,h-wallPx*2),ang,inner,255)}
...
function drawCentralComplex(buf,z,x,y){
  const ang=.02,wall=Math.max(.9,metersToPixels(z,center.lat,.62));
  const[mcx,mcy]=lngLatToTilePixel(z,x,y,center.lon,center.lat),mw=metersToPixels(z,center.lat,34),mh=metersToPixels(z,center.lat,31),sx=metersToPixels(z,center.lat,1.9),sy=metersToPixels(z,center.lat,2.3);
  drawRotRect(buf,mcx+sx,mcy+sy,mw*1.05,mh*1.05,ang,[46,37,30],105);
  drawOutlinedRect(buf,mcx,mcy,mw,mh,ang,[139,100,68],[184,155,111],wall);
  drawRotRect(buf,mcx,mcy,mw*.69,mh*.66,ang,[172,146,104],245);
  if(z>=16){
    for(let k=0;k<7;k++){const px=mcx-mw*.28+k*mw*.093;drawDisc(buf,px,mcy-mh*.29,Math.max(.7,wall*.8),[116,82,56],190)}
    drawLine(buf,mcx-mw*.36,mcy+mh*.27,mcx+mw*.36,mcy+mh*.27,Math.max(.8,wall*.9),[119,83,56],180);
  }

  // Small eastern chambers, visually separate from the mosque courtyard.
  for(let i=0;i<8;i++){
    const lon=39.61179,lat=24.46777-i*.000075,[cx,cy]=lngLatToTilePixel(z,x,y,lon,lat),w=metersToPixels(z,lat,5.1),h=metersToPixels(z,lat,4.1);
    drawRotRect(buf,cx+sx*.55,cy+sy*.6,w,h,ang,[52,41,31],90);
    drawOutlinedRect(buf,cx,cy,w,h,ang,[145,103,70],[181,143,97],Math.max(.55,wall*.45));
  }

  // Open market west of the center: deliberately no fixed rows of masonry shops.
  const[mkx,mky]=lngLatToTilePixel(z,x,y,39.60988,24.46710),mkw=metersToPixels(z,center.lat,104),mkh=metersToPixels(z,center.lat,49);
  drawRotRect(buf,mkx,mky,mkw,mkh,-.08,[188,165,124],80);
  if(z>=16){
    for(let i=0;i<14;i++){const dx=(seed(700+i*5)-.5)*mkw*.82,dy=(seed(701+i*5)-.5)*mkh*.68,[ox,oy]=rotOffset(dx,dy,-.08);drawRotRect(buf,mkx+ox,mky+oy,Math.max(1.4,metersToPixels(z,center.lat,2.2+seed(i)*2.4)),Math.max(1,metersToPixels(z,center.lat,1.4)),(seed(900+i)-.5)*.8,[124,91,59],115)}
  }

  for(const path of centralPaths){const pts=path.map(([lon,lat])=>lngLatToTilePixel(z,x,y,lon,lat)),w=metersToPixels(z,center.lat,3.0);drawPolyline(buf,pts,w*1.65,[104,87,64],40);drawPolyline(buf,pts,w,[205,183,143],150)}
}

function makeTile(z,x,y){const buf=Buffer.alloc(TILE*TILE*4);
  ...
  if(z>=15){for(const p of farmPlots){const[cx,cy]=lngLatToTilePixel(z,x,y,p.lon,p.lat),wp=metersToPixels(z,p.lat,p.w),hp=metersToPixels(z,p.lat,p.h);if(cx+wp<0||cy+hp<0||cx-wp>TILE||cy-hp>TILE)continue;const shade=90+p.tone*18;drawRotRect(buf,cx,cy,wp,hp,p.ang,[shade,105+p.tone*18,63+p.tone*8],26);const edge=Math.max(.6,metersToPixels(z,p.lat,.45)),[a1,b1]=rotOffset(-wp*.5,-hp*.5,p.ang),[a2,b2]=rotOffset(wp*.5,-hp*.5,p.ang);drawLine(buf,cx+a1,cy+b1,cx+a2,cy+b2,edge,[87,75,48],70);if(z>=16&&Math.floor(p.tone*10)%3===0){const[q1,r1]=rotOffset(-wp*.42,0,p.ang),[q2,r2]=rotOffset(wp*.42,0,p.ang);drawLine(buf,cx+q1,cy+r1,cx+q2,cy+r2,Math.max(.7,edge*.7),[79,72,48],80)}}}
...
fs.rmSync(OUT,{recursive:true,force:true});let total=0;
for(const z of[13,14,15,16,17]){const minX=lon2x(bounds.west,z),maxX=lon2x(bounds.east,z),minY=lat2y(bounds.north,z),maxY=lat2y(bounds.south,z);for(let x=minX;x<=maxX;x++)for(let y=minY;y<=maxY;y++){const dir=path.join(OUT,String(z),String(x));fs.mkdirSync(dir,{recursive:true});fs.writeFileSync(path.join(dir,`${y}.png`),makeTile(z,x,y));total++}}
console.log(`Generated ${total} historically differentiated raster tiles with central mosque, chambers, market and paths.`);
